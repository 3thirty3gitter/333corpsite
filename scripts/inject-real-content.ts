...
// Load .env.local BEFORE everything else
dotenv.config({ path: path.resolve(__dirname, '../.env.local') });

async function injectContent() {
  console.log('🚀 Injecting real KB content...');
  ...
  const articles = [
    {
      title: 'SinaLite File Preparation Guide',
      category: 'Production',
      content: `# SinaLite File Preparation Guide
...
*   Flatten all transparencies before exporting.`
    ...
    {
      title: 'Large Format Proofing Checklist',
      category: 'Quality Control',
      content: `# Large Format Proofing Checklist
...
*   [ ] Are phone numbers and QR codes tested?`
    },
    {
      ...
      category: 'Logistics',
      content: `# Shipping & Logistics Overview
...
*   **Internal Fulfillment:** Must be packed and labeled by 4 PM for next-day pickup.`
    },
    {
      ...
      category: 'HR',
      content: `# Employee Handbook 2026 (Internal Version)
...
*   **Holidays:** We observe all standard federal holidays.`
    ...
    {
      title: 'Standard Terms of Employment',
      category: 'Legal',
      content: `# Standard Terms of Employment
...
During employment and for 12 months following termination, employees agree not to solicit current customers or employees for competing business interests.`
  ...
  const trainingModules = [
    {
      title: 'PilotSuite Hub Onboarding',
      description: 'A walkthrough of the internal hub, how to access documents, and how to use the SinaLite pricing tool.',
      category: 'Onboarding',
      duration_minutes: 15
    },
    {
      title: 'Shop Safety & Production Standards',
      description: 'Overview of chemical safety for inks, machinery lock-out procedures, and PPE requirements.',
      category: 'Safety',
      duration_minutes: 45
    },
    {
      title: 'Advanced SinaLite Quoting',
      description: 'Learn how to manage shipping estimations, handling fees, and bulk markup rules.',
      category: 'Production',
      duration_minutes: 30
    }
  ];
  ...
  console.log('🧹 Clearing placeholder articles and modules...');
  await admin.from('knowledge_base_articles').delete().in('category', ['Tools', 'Pricing', 'HR']);
  await admin.from('training_modules').delete().in('category', ['Onboarding', 'Technical', 'Soft Skills']);

  // Insert KB articles
  for (const article of articles) {
    console.log(`📝 Inserting KB: ${article.title}`);
    await admin.from('knowledge_base_articles').insert(article);
  }

  // Insert Training modules
  for (const module of trainingModules) {
    console.log(`🎓 Inserting Training: ${module.title}`);
    await admin.from('training_modules').insert(module);
  }

  console.log('✅ Real content injection complete!');
}
